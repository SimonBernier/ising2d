...
std::vector<double> hvector(int, int, double, double, double, double);
...
int main(int argc, char *argv[]){
    ...
    char schar[128];
    int n1 = std::sprintf(schar,"Ly_%d_Lx_%d_h_%0.2f_tau_%0.1f_maxDim_%d_uni.dat",Ly,Lx,h,tau,maxDim);
    ...
    auto hvals = hvector(Lx, Ly, 0.0, h, tau, tanhshift);
    for(int i=1; i<=Lx; i++){
        for(int j=1; j<=Ly; j++){
            int b = Ly*(i-1) + j;
    ...
    double finalTime = 0.25*double(Lx) + 2.0*tau*tanhshift; // 0.5*N/2 + 0.5*N/v + 2*tau*shift
    ...
    for(int n=1; n<=nt; n++){
        ...
        hvals = hvector(Lx, Ly, tval, h, tau, tanhshift);
        ...
        en = tdvp(psi, H, -Cplx_i*delta1, sweeps1, {"Silent",true,"NumCenter",numCenter});
...
std::vector<double> hvector(int Lx, int Ly, double tval, double h, double tau, double tanhshift){
    ...
    for (int i = 1; i <= Lx; i++){
        ...
        for (int j = 1; j <= Ly; j++){
            ...
            int index = Ly*(i-1) + j;
            hvals[index-1] = h*(0.5 + 0.5*tanh( -tval/tau + tanhshift ));
